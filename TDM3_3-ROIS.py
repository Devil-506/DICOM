...
################################ Tracer trois formes et leurs symétries
def tracer_formes(images):
    ...
    # Ajouter 3 couleurs pour chaque ROI
    couleurs = ['red', 'green', 'blue']  # 3 couleurs au lieu de 7
    ...
    def finish_drawing(event):
        ...
        if event.inaxes is not None:
            ...
            if couleur_index >= len(couleurs):
                # Si on a fini de dessiner les 3 formes, on arrête
                plt.gcf().canvas.mpl_disconnect(cid_press)
                plt.gcf().canvas.mpl_disconnect(cid_move)
                plt.gcf().canvas.mpl_disconnect(cid_release)

                # Afficher les valeurs moyennes des 3 ROI et de leurs symétries
                print("\nMoyennes des pixels dans les 3 ROI et leurs symétries :")
                for i in range(len(moyennes_rois)):
...
if chemin_dossier:
    ...
    if images_fenetrees is not None:
        ...
        index = 85  # Index de l'image que vous voulez afficher
        image_specifique = images_fenetrees[index]

        # Dessiner 3 formes avec leurs symétries
